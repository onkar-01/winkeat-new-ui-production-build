...
const MenuCard = ({ product, handler }) => {
  const { name, user, price, images, _id, stock } = product;
  ...
  // Define the card's CSS classes based on stock availability
  const cardClasses = `lg:w-72 bg-white shadow-md rounded-xl duration-500 ${
    isOutOfStock
      ? "opacity-50 cursor-not-allowed"
      : "hover:scale-105 hover:shadow-xl"
  }`;

  // Define CSS classes for price and stock status
  const priceClasses = isOutOfStock
    ? "text-sm sm:text-lg font-semibold text-red-500"
    : "text-sm sm:text-lg font-semibold text-black cursor-auto my-3";
